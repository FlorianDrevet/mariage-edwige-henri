...
| 2026-05-11 | **Fix update cadeau admin** — Correction du transport multipart côté frontend legacy: `AxiosService` ne force plus `Content-Type` pour `FormData`, et les appels création/mise à jour cadeau n’envoient plus `multipart/form-data` manuellement. La boundary est désormais générée par le navigateur, ce qui rétablit le binding `[FromForm]` fiable du backend sur `PUT /wedding-list/{giftId}`. |
